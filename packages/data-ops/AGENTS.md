...
│   ├── relations.ts   # Drizzle relations() config (powers db.query.* relational API)
...
7. Add export to `package.json` — point it at `./src/{domain}/index.ts`
...
<important if="you are working with auth setup, session configuration, or the auth server module">
...
- Import through a path alias — this package uses relative imports only, so consumers can compile the source directly
